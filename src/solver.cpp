	...
	if (!binary)
		for (clause_t::iterator it = c.begin(); it != c.end(); ++it) {
			...
			if (LitState != LitState::L_UNSAT && *it != other_watch) { // found another watch_lit
				...
				if (is_left_watch) c.lw_set(loc);    // if literal was the left one 
				else c.rw_set(loc); // if literal was the right one				
				return ClauseState::C_UNDEF;
			}
		}
	switch (lit_state(other_watch, get_lit_state(other_watch))) {
	case LitState::L_UNSAT: // conflict
		if (verbose_now()) { c.print_real_lits(); cout << " is conflicting" << endl; }
...
void Solver::validate_assignment() {
	...
	for (vector<Clause>::iterator it = cnf.begin(); it != cnf.end(); ++it) {
		...
		for(clause_it it_c = it->cl().begin(); it_c != it->cl().end() && !found; ++it_c) 
			if (lit_state(*it_c, get_lit_state(*it_c)) == LitState::L_SAT) found = 1;
		if (!found) {
			cout << "fail on clause: "; 
			it->print();
			cout << endl;
